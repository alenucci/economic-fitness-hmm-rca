...
class RcaHmm:
    ...
    def __init__(self, series, n_states):
        ...
        self.distr = norm(loc=means[:, np.newaxis],
    ...
    def baum_welch(self, series, eps=5):
        ...
        alpha_opt, _ = np.einsum_path("ri,ij,rj->rj",
                                      alpha[..., 0],
                                      self.matrix,
                                      obs_prob[..., 0],
                                      optimize="optimal")
        beta_opt, _ = np.einsum_path("ij,rj,rj->ri",
                                     self.matrix,
                                     beta[..., 0],
                                     obs_prob[..., 0],
                                     optimize="optimal")
        matrix_opt, _ = np.einsum_path("rit,ij,rjt,rjt->ij",
                                       alpha[..., :-1],
                                       self.matrix,
                                       beta[..., 1:],
                                       obs_prob[..., 1:],
        ...
        while True:
            ...
            try:
                if np.abs(self.lk[-2] - self.lk[-1]) > 10**-eps:
                    ...
                    if (order != np.arange(self.n_states)).any():
                        print("Sorted")
                        self.distr = norm(loc=self.distr.mean[order],
            ...
            means = (gamma_mix[..., 0, :] *
                     series[:, np.newaxis, :]).sum(axis=(0, 2))
            means /= gamma_mix[..., 0, :].sum(axis=(0, 2))
            devs = ((series[:, np.newaxis] - means[:, np.newaxis])**2 *
                    gamma_mix[..., 0, :]).sum(axis=(0, 2))
...
def import_rca_data(data_path):
    ...
    with open(
            data_path + str(first_year) + "/countries" + str(first_year) +
            ".txt", "r") as countries_file:
        countries = {
            country.rstrip(): i
...
def save_country_model(country, country_model, country_series):
    ...
    np.savetxt(save_path + "means and devs.txt", country_model.distr_params)
    ...
    np.savetxt(save_path + "states.txt",
